...
class API(object):
    ...
    def __call__(self, event, context):
        ...
        try:
            route_entry.data['request_obj'].args = request_params.copy()
            route_entry.data['request_obj'].data = ''
            if http_method == "POST":
                route_entry.data['request_obj'].data = event.get("body", '')

            route_entry.data['request_obj'].remote_addr = event['requestContext']['identity']['sourceIp']
            route_entry.data['request_obj'].headers = headers

            response = route_entry.endpoint(data=route_entry.data, **function_kwargs)
